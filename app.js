...
const els = {
  ...
  fullscreenDrawingBtn: $("#fullscreenDrawingBtn"),
  exitFullscreenDrawingBtn: $("#exitFullscreenDrawingBtn"),
...
function showDayDetails(key, selectedButton) {
  ...
  if (hasHandwriting) {
    ...
    heading.className = "day-notes";
    heading.innerHTML = "<strong>Handwritten notes</strong>";
    els.dayDetail.appendChild(heading);
    renderHandwritingPreview(heading, state.handwriting[key]);
  }
}
...
let pencilRecentlyUsed = false;
let pencilResetTimer = null;
let drawingFrameRequested = false;
...
function canvasPoint(event) {
  ...
  return {
    ...
    pressure: event.pointerType === "pen"
      ? Math.max(.08, Math.min(1, event.pressure || .35))
      : .42,
    t: performance.now()
  };
}
...
function strokeWidth(point, tool) {
  if (tool === "eraser") return 30;
  return 1.25 + Math.pow(point.pressure || .35, .8) * 4.6;
}
...
function drawStroke(ctx, stroke, width, height) {
  ...
  if (points.length === 1) {
    const p = points[0];
    ctx.beginPath();
    ctx.arc(p.x * width, p.y * height, strokeWidth(p, stroke.tool) / 2, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
    return;
  }
  ...
  for (let i = 1; i < points.length - 1; i++) {
    ...
    ctx.lineWidth = strokeWidth(current, stroke.tool);
    ctx.quadraticCurveTo(current.x * width, current.y * height, midX, midY);
  }

  const last = points[points.length - 1];
  ctx.lineWidth = strokeWidth(last, stroke.tool);
  ctx.lineTo(last.x * width, last.y * height);
  ctx.stroke();
  ctx.restore();
}

function renderStrokeCollection(canvas, strokes) {
  const ctx = canvas.getContext("2d");
  const ratio = Math.min(2, window.devicePixelRatio || 1);
...
function renderDrawing() {
  renderStrokeCollection(els.handwritingCanvas, todayStrokes());
  els.canvasHint.parentElement.classList.toggle("has-ink", todayStrokes().length > 0);
}

function requestDrawingRender() {
  if (drawingFrameRequested) return;
  drawingFrameRequested = true;
  requestAnimationFrame(() => {
    drawingFrameRequested = false;
    renderDrawing();
  });
}
...
function shouldIgnorePointer(event) {
  if (event.pointerType === "pen") {
    pencilRecentlyUsed = true;
    clearTimeout(pencilResetTimer);
    pencilResetTimer = setTimeout(() => { pencilRecentlyUsed = false; }, 700);
    return false;
  }

  // Once Pencil is active, ignore touch contacts to reduce palm interference.
  if (event.pointerType === "touch" && pencilRecentlyUsed) return true;
  return false;
}

function beginStroke(event) {
  if (noteMode !== "write" || shouldIgnorePointer(event)) return;
  event.preventDefault();
  els.handwritingCanvas.setPointerCapture?.(event.pointerId);
  els.handwritingCanvas.parentElement.classList.add("pencil-active");
...
function continueStroke(event) {
  if (!activeStroke || event.pointerId !== activeStroke.pointerId || shouldIgnorePointer(event)) return;
  event.preventDefault();
  ...
  events.forEach(item => {
    ...
    // Ignore microscopic jitter while preserving detail.
    if (dx * dx + dy * dy > 0.0000015) activeStroke.points.push(point);
  });
  requestDrawingRender();
}
...
function endStroke(event) {
  ...
  event.preventDefault();
  activeStroke = null;
  els.handwritingCanvas.parentElement.classList.remove("pencil-active");
  scheduleDrawingSave();
}

function enterDrawingFullscreen() {
  els.drawingWorkspace.classList.add("fullscreen");
...
function exitDrawingFullscreen() {
  els.drawingWorkspace.classList.remove("fullscreen");
...
function renderHandwritingPreview(container, strokes) {
  const preview = document.createElement("div");
  preview.className = "handwriting-preview";
  ...
  requestAnimationFrame(() => {
    ...
    renderStrokeCollection(canvas, strokes);
  });
}
...
els.fullscreenDrawingBtn.addEventListener("click", enterDrawingFullscreen);
els.exitFullscreenDrawingBtn.addEventListener("click", exitDrawingFullscreen);
...
document.addEventListener("keydown", event => {
  if (event.key === "Escape" && els.drawingWorkspace.classList.contains("fullscreen")) {
    exitDrawingFullscreen();
  }
});
